...
// @grant        GM_openInTab
// @connect      imdb.com
// @resource     imdbIcon   https://raw.githubusercontent.com/ArmineZ/netflix-imdb/master/imdb-icon.png
// ==/UserScript==

(function() {
    "use strict";

    GM_addStyle(".imdb-rating{display:-webkit-inline-box;display:-webkit-inline-flex;display:-moz-inline-box;display:-ms-inline-flexbox;display:inline-flex;-webkit-flex-wrap:nowrap;-ms-flex-wrap:nowrap;flex-wrap:nowrap;-webkit-box-align:center;-webkit-align-items:center;-moz-box-align:center;-ms-flex-align:center;align-items:center;color:#fff;padding:8px 0}.imdb-image{width:28px;height:28px}.imdb-error,.imdb-loading,.imdb-no-rating,.imdb-score,.imdb-votes{margin:0 3px}.imdb-overlay{position:absolute;top:10px;right:10px;z-index:100}");
    ...
    function imdbRenderingForCard(node) {
        var bobContainer = node.querySelector(".bob-container");

        if (bobContainer.innerHTML.length < 10) {
            var titleNode = node.querySelector(".fallback-text");
            var title = titleNode && titleNode.innerHTML;
            if (!title) return;
            var ratingNode = getRatingNode(title);

            bobContainer.appendChild(ratingNode);
        }
    }
	...
	function updateCards() {
		...
		for (var i = 0; i < cards.length; i++) {
			...
			if (newNode.classList.contains("title-card-container")) {
